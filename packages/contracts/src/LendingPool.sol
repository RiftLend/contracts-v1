...
contract LendingPool is Initializable, LendingPoolStorage, SuperPausable {
  ...
  function deposit(
    ...
    (bool isFirstDeposit, uint256 mintMode, uint256 amountScaled) = IRToken(
      rToken
  ...
  function withdraw(
    ...
    (uint256 mode, uint256 amountScaled) = IRToken(rToken).burn(
      sender,
      to,
      toChainId,
      amountToWithdraw,
  ...
  function repay(
    ...
    (mode, amountBurned) = IVariableDebtToken(reserve.variableDebtTokenAddress)
    ...
    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      ...
      if (
        ...
        _reserves[vars.currentAsset].cumulateToLiquidityIndex(
          IERC20(vars.currentrTokenAddress).totalSupply(),
          vars.currentPremium
        );
  ...
  function finalizeTransfer(
    ...
    require(
      msg.sender == _reserves[asset].rTokenAddress,
      Errors.LP_CALLER_MUST_BE_AN_RTOKEN
    );
  ...
  function getTokenType() internal view returns (address, uint256 token_type) {
    (bytes32 lendingPool, address rVaultAsset) = _addressesProvider
      .getRVaultAsset();
