...
contract LendingPool is Initializable, LendingPoolStorage, SuperPausable {
  ...
  bytes2 public constant DEPOSIT_WITHDRAW_UPDATE_STATE_MASK = bytes2(uint16(3));
...
error ZeroParams();
 ...
 // Update states with unchecked for gas optimization where safe
  ...
  function withdraw(
    ...
    require(
      block.timestamp >=
        _lastWithdrawalTime[sender] + WITHDRAW_COOL_DOWN_PERIOD,
    ...
    _updateStates(
      ...
      0,
      amountToWithdraw,
      DEPOSIT_WITHDRAW_UPDATE_STATE_MASK
    );
