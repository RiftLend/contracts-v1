...
contract LendingPool is Initializable, LendingPoolStorage, SuperPausable {
  ...
  function _onlyRouterOrSelf() internal view {
    require(
      _addressesProvider.getRouter() == msg.sender ||
 ...
 */
  ...
  function deposit(
    address sender,
    address asset /* underlying or superAsset or Rvault*/,
    uint256 amount,
    address onBehalfOf,
    uint16 referralCode
  ) external  onlyRouter {

    // Since there are two scenarios for deposit asset type, 
    // - underlying asset 
    // - SuperAsset
    // - Rvault
    ...
    // Transfer the asset of worth `amount` to the contract
    ...
    (bool isFirstDeposit, uint256 mintMode, uint256 amountScaled) = IAToken(
      aToken
    ).mint(onBehalfOf, amount, reserve.liquidityIndex);
  ...
  function withdraw(
    ...
    _lastWithdrawalTime[sender] = block.timestamp;
    ...
    (uint256 mode, uint256 amountScaled) = IAToken(aToken).burn(
      sender,
      to,
      toChainId,
      amountToWithdraw,
  ...
  function repay(
    ...
    if (interestRateMode == DataTypes.InterestRateMode.STABLE) {
      (mode, amountBurned) = IStableDebtToken(reserve.stableDebtTokenAddress)
        .burn(onBehalfOf, paybackAmount);
    } else {
      (mode, amountBurned) = IVariableDebtToken(
        reserve.variableDebtTokenAddress
      ).burn(onBehalfOf, paybackAmount, reserve.variableBorrowIndex);
    }
  ...
  function swapBorrowRateMode(
    ...
    if (interestRateMode != DataTypes.InterestRateMode.STABLE) {
      (, stableDebtAmount) = IStableDebtToken(reserve.stableDebtTokenAddress)
        .burn(sender, stableDebt);
      (, , variableDebtAmount) = IVariableDebtToken(
        reserve.variableDebtTokenAddress
      ).mint(sender, sender, stableDebt, reserve.variableBorrowIndex);
    } else {
      (, variableDebtAmount) = IVariableDebtToken(
        reserve.variableDebtTokenAddress
      ).burn(sender, variableDebt, reserve.variableBorrowIndex);
      (, , stableDebtAmount) = IStableDebtToken(reserve.stableDebtTokenAddress)
        .mint(sender, sender, variableDebt, reserve.currentStableBorrowRate);
    }
  ...
  function rebalanceStableBorrowRate(
    ...
    (, uint256 amountBurned) = IStableDebtToken(address(stableDebtToken)).burn(
      user,
      stableDebt
    );
    (, , uint256 amountMinted) = IStableDebtToken(address(stableDebtToken))
  ...
  function flashLoan(
    ...
    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      ...
      if (
        ...
        ISuperAsset(vars.currentAsset).mint(
          vars.currentATokenAddress,
          vars.currentAmountPlusPremium
        );
      } else {
  ...
  function getReserveNormalizedVariableDebt(
    ...
    return
      _reserves[superAsset].getNormalizedDebt();
  }
  ...
  function _executeBorrow(ExecuteBorrowParams memory vars) internal {
    ...
    uint256 amountInETH = (IPriceOracleGetter(oracle).getAssetPrice(
      vars.asset
    ) * vars.amount) / 10 ** reserve.configuration.getDecimals();
    address superAsset = _addressesProvider.getSuperAsset();
    ...
    if (
      ...
      (isFirstBorrowing, mintMode, amountScaled) = IStableDebtToken(
        reserve.stableDebtTokenAddress
      ).mint(vars.user, vars.onBehalfOf, vars.amount, currentStableRate);
    } else {
      (isFirstBorrowing, mintMode, amountScaled) = IVariableDebtToken(
        reserve.variableDebtTokenAddress
      ).mint(
          vars.user,
          vars.onBehalfOf,
