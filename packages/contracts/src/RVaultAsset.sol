...
import {ISuperAssetAdapter} from './interfaces/ISuperAssetAdapter.sol';
...
contract RVaultAsset is SuperOwnable, OFT {
  ...
  function _bridgeCrossCluster(
    ...
    if (pool_type == 1) {
      ...
      bytes memory compose_message = OFTLogic.encodeMessage(
      ...
      (MessagingReceipt memory msgReceipt, ) = ISuperAssetAdapter(
        superAssetAdapter
      ).send(sendParam, fee, payable(address(this)));
      ...
      // decide recipient
      if (
        chainIdToClusterType[toChainId] ==
        DataTypes.Chain_Cluster_Types.SUPER_CHAIN
      ) {
  ...
  function lzReceive(
    ...
    if (superAssetAdapter != msg.sender && address(endpoint) != msg.sender) {
    ...
    (
      ...
      address oftTxCaller
    ) = OFTLogic.decodeMessage(_message);
  ...
  function _bridge(
    ...
    DataTypes.Chain_Cluster_Types sourceType = chainIdToClusterType[
      block.chainid
    ];
    DataTypes.Chain_Cluster_Types destinationType = chainIdToClusterType[
      toChainId
    ];
    DataTypes.Chain_Cluster_Types super_chain_type = DataTypes
      .Chain_Cluster_Types
      .SUPER_CHAIN;
    DataTypes.Chain_Cluster_Types other_cluster_type = DataTypes
      .Chain_Cluster_Types
      .OTHER;

    if (sourceType == super_chain_type) {
      if (destinationType == super_chain_type && isSuperTokenBridgeEnabled) {
        _bridgeUsingSuperTokenBridge(
          amount,
          receiverOfUnderlying,
          underlying,
          toChainId
      ...
      sourceType == other_cluster_type && destinationType == other_cluster_type
    ) {
      _bridgeCrossCluster(amount, receiverOfUnderlying, toChainId);
    } else {
  ...
  function setChainClusterType(
    uint256 chainId,
    DataTypes.Chain_Cluster_Types cluster_type
  ...
  function setWithdrawCoolDownPeriod(
    uint256 _newPeriod
  ) external onlySuperAdmin {
  ...
  function _setOwner(
    address newOwner
  ) internal override(Ownable, SuperOwnable) {
  ...
  ) public payable override(SuperOwnable, Ownable) {
