...
contract LendingPoolDeployer is Script {
    ...
    function run() public {
        ...
        relayer = vm.parseTomlAddress(deployConfig, ".relayers.address1");
    ...
    function deployFullLendingPool() public broadcast {
        ...
        RVaultAsset(payable(deployedContracts.rVaultAsset)).setAllLimits(
            lzReceiveGasLimit, lzComposeGasLimit, maxDepositLimit
        );

        RVaultAsset(payable(deployedContracts.rVaultAsset)).initialize(
        ...
        lpAddressProvider.setRelayerStatus(relayer, true);
    ...
    function deployUnderlying() internal returns (address) {
        return _deployWithCreate2(
            ...
            abi.encode(
                ...
                vm.parseTomlUint(deployConfig, ".owner.address")
            )
        );
    }
